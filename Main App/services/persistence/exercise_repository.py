...
def get_user(username: str,password: str) -> sqlite3.Row:
    conn = _get_connection()
...
def add_exercise(user_id, exercise_name, reps, sets, time):
    ...
    with conn:
        ...
        if existing:
            conn.execute("""
                         UPDATE exercises SET reps = reps+ ? , sets = sets + ? , time = time + ? WHERE id = ? """,(reps,sets,time,existing['id']))
            
        else:
            conn.execute("""
                         INSERT INTO exercises (user_id,exercise_name,sets,reps,time) VALUES (? , ? , ? , ? , ?)""",(user_id,exercise_name,sets,reps,time))
